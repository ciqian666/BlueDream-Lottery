...
public class ParticleTask {
    ...
    private volatile double ticks = 0;
    ...
    public void run() {
        ticks += 0.5;
        final double currentTicks = ticks;
        ...
        for (Map.Entry<Location, String> entry : blocks.entrySet()) {
            ...
            // 每个抽奖方块位置调度到其所在区域执行渲染，兼容 Folia 的多区域线程
            plugin.getFoliaLib().getImpl().runAtLocation(loc, task -> {
                LotteryPool pool = manager.getPool(poolName);
                if (pool == null) return;
                ...
                if (!anyPlayerNearby) return;

                String pType = pool.getParticleType();
                if (pType == null || pType.equals("NONE")) return;

                renderEffect(loc, pType, currentTicks);
            });
        }
    }

    private void renderEffect(Location blockLoc, String type, double currentTicks) {
        ...
        switch (type.toUpperCase()) {
            case "VORTEX":
                renderVortex(center, currentTicks);
                break;
            case "HALO":
                renderHalo(center, currentTicks);
                break;
            case "STAR_DUST":
                renderStarDust(center, currentTicks);
                break;
            case "FLAME_RING":
                renderFlameRing(center, currentTicks);
                break;
            case "MAGIC_AURA":
                renderMagicAura(center, currentTicks);
                break;
        }
    }

    private void renderVortex(Location center, double currentTicks) {
        double radius = 0.8;
        for (int i = 0; i < 2; i++) {
            double angle = currentTicks * 0.2 + (i * Math.PI);
            double x = Math.cos(angle) * radius;
            double z = Math.sin(angle) * radius;
            double y = (Math.sin(currentTicks * 0.1 + (i * Math.PI)) * 0.5) + 0.5;
    ...
    private void renderHalo(Location center, double currentTicks) {
        double radius = 0.6;
        for (int i = 0; i < 5; i++) {
            double angle = (currentTicks * 0.15) + (i * (Math.PI * 2 / 5));
            double x = Math.cos(angle) * radius;
            double z = Math.sin(angle) * radius;
            double y = 1.2 + Math.sin(currentTicks * 0.1) * 0.1;
    ...
    private void renderStarDust(Location center, double currentTicks) {
        if (currentTicks % 2 == 0) {
            for (int i = 0; i < 3; i++) {
    ...
    private void renderFlameRing(Location center, double currentTicks) {
        double radius = 0.7 + Math.sin(currentTicks * 0.1) * 0.1;
    ...
    private void renderMagicAura(Location center, double currentTicks) {
        double radius = 0.5;
        double angle = currentTicks * 0.2;
